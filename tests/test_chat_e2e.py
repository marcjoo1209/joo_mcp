...
def test_chat_crud_end_to_end(chat_client):
    """자연어로 생성 → 조회 흐름을 검증한다.

    실제 Gemini 가 (영속) MCP 세션의 도구(create_note/list_notes)를 호출하고,
    ...
    notes = chat_client.get("/notes").json()
    assert len(notes) >= 1

    # 3) 목록 조회 요청도 정상 응답
    res2 = chat_client.post("/chat", json={"message": "메모 목록 보여줘"})
